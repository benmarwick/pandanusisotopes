...
iso_results_all <- readxl::read_excel(here::here("analysis/data/raw_data/IsotopeResults-Nicholas-Patton.xlsx"),
                                                 sheet = "Site Averaged_ABA")
# 259 measurements

# keep 'woodland' only for locations that are Arnhem Highway, ERA Mining Lease
# keep 'Pandanus Fringe' aka 'floodplain' for locations that are not Stuart Highway

iso_results <-
  iso_results_all %>%
  filter(Species == "Pandanus spiralis") %>%
  filter(`Physical pre-treatment` ==  "Charred at 500ºC for 2 hours") %>%
  filter(Locality == "Arnhem Highway, ERA Mining Lease")
# 91 measurements

iso_results_veg <-
  iso_results %>%
  filter(Vegetation %in% c("Open forest and woodland",
                           "Seasonal causeway",
                           "Pandanus fringe") ) %>%
...
x <- (iso_results$`δ13CV-PDB (‰)`)

iqr = c(quantile(x, 3/4), quantile(x, 1/4))
...
Plot the plant isotope values, line plot by depth
...
phase_boxes +
  ...
  #             filter(phase != "3"),
  #           aes(mean_depth, -25., label = phase_chr)) +
  geom_point(data = d13_mjb_without_4a_and_4b , 
               aes(depth_by_context, 
                   values,
                   group = depth_by_context),
                size = 2,
                alpha = 0.2) +
  geom_line(data = d13_mjb_without_4a_and_4b %>% 
              group_by(depth_by_context) %>% 
...
Results of the modern stable carbon isotope analysis.

Linear relationship is observed between precipitation and δ13C values of P. spiralis with 95% confidence intervals (grey area) and 95% prediction intervals (dashed lines) when removing all sites affected by microclimate effects (blue dots with mean evaporation <5 mm per day).

```{r}
# I didn't do the original version of this, I just recreate it here for verification. 
# I guess Nick did it?

mean_annual_precip_data <- 
  readxl::read_excel(here::here("analysis/data/raw_data/IsotopeResults-Nicholas-Patton.xlsx"),
                     sheet = "Site Averaged_ABA")

ggplot(mean_annual_precip_data) +
  aes(`Mean Annual Rainfall (mm)`,
      `δ13CV-PDB (‰)`,
      colour = `Mean Daily Evaporation (mm)` ,
      ymin = `δ13CV-PDB (‰)` - `Standard Deviation δ13CV-PDB (‰)`,
      ymax = `δ13CV-PDB (‰)` + `Standard Deviation δ13CV-PDB (‰)`) +
  geom_pointrange() +
  geom_smooth(data = mean_annual_precip_data %>% 
                       filter( `Mean Daily Evaporation (mm)` > 5.5), 
              aes(`Mean Annual Rainfall (mm)`,
                  `δ13CV-PDB (‰)`),
              method = "lm")
...
mean_annual_precip_data %>% 
  filter( `Mean Daily Evaporation (mm)` > 5.5) %>%  
  lm(  `δ13CV-PDB (‰)` ~ `Mean Annual Rainfall (mm)` , data = .) %>% 
  broom::tidy()

res <- lm(`δ13CV-PDB (‰)` ~ `Mean Annual Rainfall (mm)`, 
   data = mean_annual_precip_data)

RMSE <- sqrt(c(crossprod(res$residuals)) / length(res$residuals))
